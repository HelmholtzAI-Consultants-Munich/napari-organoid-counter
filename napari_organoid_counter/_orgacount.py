...
class OrganoiDL():
    ...
    def run(self, 
        ...
        self.pred_ids = [i for i in range(num_predictions)]
    ...
    def update_bboxes_scores(self, new_bboxes, new_scores, new_ids):
        #### MAKE SURE FIRST THAT BOXES ARE CONVERTED TO TENSORS
        ...
        if self.pred_bboxes is None:
            ...
            self.pred_ids = torch.cat((self.pred_ids, new_ids[added_ids]))
            ...
            for idx in potential_removed_ids:
                ...
                if self.pred_bboxes[idx] > self.cur_confidence and dx > min_diameter_x and dy > min_diameter_y:
            ...
            for idx in reversed(remove_ids):
                ...
                self.pred_ids = torch.cat((self.pred_ids[:,:idx], self.pred_ids[:,idx+1:]))
